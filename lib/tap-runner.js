  ...
  chain(files.map(function(f) {
    return function (cb) {
      ...
      fs.lstat(f, function(er, st) {
        ...
        if (self.doCoverage) {
          if (self.doCoverage === 'cover') {
            args.unshift('run')
            cmd = '/Users/jmorrison/github/jrgm/node-tap/node_modules/.bin/cover'
          } else if (self.doCoverage === 'istanbul') {
            args.unshift('cover')
            cmd = '/Users/jmorrison/github/jrgm/node-tap/node_modules/.bin/istanbul'
          }
        }

        //console.error('cp.spawn', cmd, args, relDir);
